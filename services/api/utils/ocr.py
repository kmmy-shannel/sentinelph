...
Engine: pytesseract (wraps the native Tesseract OCR binary). See the
Co-Developer Onboarding & Runbook section of AI_SCAM_DETECTION_GUIDE.md
...
def extract_text_from_image(
    ...
    error) if the image decodes fine but Tesseract finds no text — that's
    a valid outcome for e.g. a blank or purely graphical screenshot.
    """
...
def extract_text_from_base64(b64_string: str) -> str:
    ...
    `data:image/...;base64,` URI header, as sent by browsers/mobile apps)
    and run OCR on it.
    """
    if not b64_string:
