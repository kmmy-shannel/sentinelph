...
/**
 * Maps a Report document from services/api/models/Report.js to the flat
 * shape the review table + vote modal expect. All original fields are
 * preserved; evidenceImage and evidenceText are new.
 */
...
export default function ReviewQueue() {
  ...
  const loadReports = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await fetchReports({ status: "pending", limit: 100 });
  ...
  useEffect(() => {
    loadReports();
    // Poll so newly-synced offline reports appear without a manual refresh.
  ...
  async function handleSubmitVote() {
    ...
    try {
      ...
      // Optimistic local update; next poll reconciles with the server.
      setReports((prev) =>
        prev.map((r) =>
          r.id === voteModal.id
            ? {
                ...
                rawStatus: decision === "approve" ? "approved" : "rejected",
              }
            : r
        )
      );
    ...
    <div>
      ...
      {voteModal && (
        <div style={{ position: "fixed", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", zIndex: 50, background: "rgba(0,0,0,0.7)", backdropFilter: "blur(4px)" }}>
          <div style={{ width: "100%", maxWidth: "560px", maxHeight: "90vh", overflowY: "auto", margin: "0 16px", borderRadius: "20px", padding: "28px", position: "relative", background: "#0e0e18", border: "1px solid #1a1a2a", boxShadow: "0 40px 80px rgba(0,0,0,0.5)" }}>
