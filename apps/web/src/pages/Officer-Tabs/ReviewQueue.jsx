...
export default function ReviewQueue() {
  ...
  const filtered = ALL_REPORTS.filter((r) => {
    if (tab === "pending")   return r.priorVotes === "None";
    if (tab === "escalated") return false;
    return true;
  });
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: "4px", marginBottom: "20px" }}>
        {[{ id: "all", label: "All" }, { id: "pending", label: "Pending", count: 7 }, { id: "escalated", label: "Escalated" }].map((t) => (
          <button key={t.id} onClick={() => setTab(t.id)}
            ...
            {t.label}
            {t.count && <span style={{ padding: "1px 6px", borderRadius: "999px", fontSize: "9px", fontWeight: 700, background: "#ef4444", color: "#fff" }}>{t.count}</span>}
          </button>
        ))}
      </div>
      ...
      <div style={{ borderRadius: "12px", overflow: "hidden", background: "#0e0e18", border: "1px solid #1a1a2a" }}>
        <div style={{ padding: "12px 20px", borderBottom: "1px solid #1a1a2a" }}>
          ...
          <div style={{ fontSize: "11px", marginTop: "2px", color: "#4b5563" }}>Click a row to expand and vote</div>
        </div>
        ...
        <div style={{ position: "fixed", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", zIndex: 50, background: "rgba(0,0,0,0.7)", backdropFilter: "blur(4px)" }}
          ...
          <div style={{ width: "100%", maxWidth: "480px", margin: "0 16px", borderRadius: "20px", padding: "28px", position: "relative", background: "#0e0e18", border: "1px solid #1a1a2a", boxShadow: "0 40px 80px rgba(0,0,0,0.5)" }}
            ...
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: "20px" }}>
              ...
              <button onClick={closeModal} style={{ color: "#4b5563", background: "none", border: "none", cursor: "pointer", fontSize: "16px" }}>✕</button>
            </div>
            ...
            <button disabled={!decision || !comment.trim()} onClick={closeModal}
