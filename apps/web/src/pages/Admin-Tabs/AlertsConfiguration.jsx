// apps/web/src/pages/Analyst-Tabs/AlertsConfiguration.jsx
...
export default function AlertsConfiguration() {
  ...
  function handleSubmit() {
    if (!metric || !recommendation.trim()) return;
    const newRec = {
      id: `REC-0${12 + pending.length - 1}`,
      metric,
      rec: recommendation,
      submitted: "Aug 28",
      analyst: "A. Mercado",
    };
    setPending(prev => [newRec, ...prev]);
    closeModal();
    setShowSuccess(true);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
      ...
      <div style={card}>
        ...
        <div style={{ fontSize: "11px", color: "#4b5563", marginBottom: "16px" }}>{pending.length} submitted change{pending.length !== 1 ? "s" : ""} awaiting officer approval</div>
        {pending.map((p) => (
        ...
        <button onClick={() => setShowModal(true)}
          style={{ width: "100%", padding: "10px", borderRadius: "10px", fontSize: "12px", fontWeight: 500, background: "#111120", border: "1px dashed #1a1a2a", color: "#6b7280", cursor: "pointer", marginTop: "4px" }}>
        ...
        <div style={{ position: "fixed", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", zIndex: 50, background: "rgba(0,0,0,0.7)", backdropFilter: "blur(4px)" }}>
          <div style={{ width: "100%", maxWidth: "520px", margin: "0 16px", borderRadius: "20px", padding: "28px", position: "relative", background: "#0e0e18", border: "1px solid #1a1a2a", boxShadow: "0 40px 80px rgba(0,0,0,0.5)" }}>
            ...
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: "20px" }}>
              <div>
                <div style={{ fontWeight: 700, color: "#fff", fontSize: "16px" }}>Submit New Recommendation</div>
                <div style={{ fontSize: "11px", marginTop: "4px", color: "#4b5563" }}>This will be sent to the NBI Officer / Project Manager for review.</div>
              </div>
            ...
            <div style={{ marginBottom: "16px" }}>
              <div style={{ fontSize: "10px", marginBottom: "8px", color: "#6b7280", fontFamily: "'JetBrains Mono',monospace", letterSpacing: "0.06em" }}>TARGET METRIC</div>
              <select value={metric} onChange={(e) => setMetric(e.target.value)}
                style={{ width: "100%", padding: "12px 16px", borderRadius: "10px", fontSize: "13px", background: "#080810", border: "1px solid #1a1a2a", color: "#e2e8f0", outline: "none", boxSizing: "border-box", cursor: "pointer" }}>
            ...
            <div style={{ marginBottom: "20px" }}>
              <div style={{ fontSize: "10px", marginBottom: "8px", color: "#6b7280", fontFamily: "'JetBrains Mono',monospace", letterSpacing: "0.06em" }}>RECOMMENDATION</div>
              <textarea value={recommendation} onChange={(e) => setRecommendation(e.target.value)}
                placeholder="Describe your recommended change and its reasoning…" rows={4}
                style={{ width: "100%", padding: "12px 16px", borderRadius: "10px", fontSize: "13px", outline: "none", resize: "none", background: "#080810", border: "1px solid #1a1a2a", color: "#e2e8f0", boxSizing: "border-box", fontFamily: "inherit" }} />
            </div>
            ...
            <div style={{ display: "flex", gap: "10px" }}>
              ...
              <button onClick={handleSubmit} disabled={!metric || !recommendation.trim()}
                style={{ flex: 1, padding: "12px", borderRadius: "10px", fontSize: "13px", fontWeight: 600, background: metric && recommendation.trim() ? "#a855f7" : "#111118", border: "none", color: metric && recommendation.trim() ? "#fff" : "#374151", cursor: metric && recommendation.trim() ? "pointer" : "not-allowed" }}>
                Submit
              </button>
            </div>
          </div>
