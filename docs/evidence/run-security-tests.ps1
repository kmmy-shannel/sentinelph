...
$resp = Invoke-Api -Uri "$API/api/v1/auth/confirm-password-reset" -Method Post -Body $body
...
$resp = Invoke-Api -Uri "$API/api/v1/auth/request-password-otp" -Method Post -Body $body
...
Expected: 400 (or non-bypass response)
Actual Status: $($resp.StatusCode)
Actual Body:
$($resp.Body)
"@
...
$envReport += "`n--- .env.example MUST be tracked (prints nothing) ---`n"
...
$httpsReport += "`n--- HTTP to HTTPS redirect ---`n"
...
Expected: scheme must be 'mongodb+srv://'
"@
